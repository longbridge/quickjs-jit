...
2. Tier 2 feedback and SSA now admit stable object arguments alongside numeric
   ...
   header are reused for the same source until a reentrant effect or element
   store invalidates the fact. Dynamic bounds checks remain where no range
   proof exists. Wrong classes, Proxy objects, unsupported values, and numeric
   mismatches deoptimize to exact interpreter state.
...
from clean source commit `fcc1a1db823beb923e9f0262faeb05e7b40077b5` using the
...
3,373,593 ns versus 4,885,385 ns for the interpreter, a 1.45x speedup. Across
