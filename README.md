...
Dependencies that still name the `rquickjs` package, including LLRT, can use
the bundled [compatibility facade](compat/rquickjs/README.md). The consuming
workspace must select it with a root-level `[patch.crates-io]` entry.
