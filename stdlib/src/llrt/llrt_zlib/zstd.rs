...
fn zstd_converter<'js>(
    ...
    let mut level = crate::llrt_compression::zstd::DEFAULT_COMPRESSION_LEVEL;
    ...
    let dst = match command {
        ZstdCommand::Compress => read_to_end_limited(
            &ctx,
            crate::llrt_compression::zstd::encoder(src, level)?,
            limit,
            src.len(),
        )?,
        ZstdCommand::Decompress => read_to_end_limited(
            &ctx,
            crate::llrt_compression::zstd::decoder(src)?,
            limit,
            src.len(),
        )?,
    };
