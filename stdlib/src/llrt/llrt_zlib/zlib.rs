...
use crate::llrt_buffer::Buffer;
use crate::llrt_context::CtxExtension;
use crate::llrt_utils::{bytes::ObjectBytes, object::ObjectExt, result::ResultExt};
use rquickjs::{
    prelude::{Opt, Rest},
...
fn zlib_converter<'js>(
    ...
    let mut level = crate::llrt_compression::zlib::Compression::default();
    if let Some(options) = options.0.as_ref() {
        if let Some(opt) = options.get_optional("level")? {
            level = crate::llrt_compression::zlib::Compression::new(opt);
        }
    }
    ...
    let dst = match command {
        ZlibCommand::Deflate => read_to_end_limited(
            &ctx,
            crate::llrt_compression::zlib::encoder(src, level),
            limit,
            src.len(),
        )?,
        ZlibCommand::DeflateRaw => read_to_end_limited(
            &ctx,
            crate::llrt_compression::deflate::encoder(src, level),
            limit,
            src.len(),
        )?,
        ZlibCommand::Gzip => read_to_end_limited(
            &ctx,
            crate::llrt_compression::gz::encoder(src, level),
            limit,
            src.len(),
        )?,
        ZlibCommand::Inflate => read_to_end_limited(
            &ctx,
            crate::llrt_compression::zlib::decoder(src),
            limit,
            src.len(),
        )?,
        ZlibCommand::InflateRaw => read_to_end_limited(
            &ctx,
            crate::llrt_compression::deflate::decoder(src),
            limit,
            src.len(),
        )?,
        ZlibCommand::Gunzip => read_to_end_limited(
            &ctx,
            crate::llrt_compression::gz::decoder(src),
            limit,
            src.len(),
        )?,
    };
