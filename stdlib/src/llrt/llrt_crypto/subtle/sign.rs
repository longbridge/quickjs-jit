...
use crate::llrt_crypto::provider::{CryptoProvider, HmacProvider};
use crate::llrt_utils::bytes::ObjectBytes;
...
fn sign(
    ...
    Ok(match algorithm {
        SigningAlgorithm::Ecdsa { hash } => {
            ...
            let digest = crate::llrt_crypto::subtle::digest::digest(hash, data);

            crate::llrt_crypto::CRYPTO_PROVIDER
