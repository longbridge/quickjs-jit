...
use crate::llrt_utils::option::Undefined;
use rquickjs::{
    class::Trace,
    prelude::{Opt, This},
...
pub(crate) fn sink_write_algorithm<'js>(
    ...
    if stream.backpressure {
        ...
        if let Some(bp_promise) = bp_promise {
            ...
            return crate::llrt_stream_web::utils::promise::upon_promise::<Value<'js>, _>(
                ctx.clone(),
                bp_promise,
                move |ctx, _| {
...
pub(crate) fn sink_close_algorithm<'js>(
    ...
    let sc = stream_class.clone();
    let cc = controller_class.clone();
    crate::llrt_stream_web::utils::promise::upon_promise::<Value<'js>, _>(
        ctx.clone(),
        flush_promise,
        move |ctx, result| {
            ...
            match result {
                Ok(_) => {
                    ...
                    let readable_controller = stream.readable.as_ref().and_then(|readable| {
                        ...
                        if let crate::llrt_stream_web::readable::ReadableStreamControllerClass::ReadableStreamDefaultController(c) = &r.controller {
                            Some(c.clone())
                        } else {
                            None
                        }
                    ...
                    if let Some(c) = readable_controller {
                        crate::llrt_stream_web::readable::readable_stream_default_controller_close_stream(
                            ctx.clone(),
                            c,
                        )?;
                    }
