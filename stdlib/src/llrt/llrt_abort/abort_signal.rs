...
use crate::llrt_events::{Emitter, EventEmitter, EventList};
use crate::llrt_exceptions::{DOMException, DOMExceptionName};
use crate::llrt_utils::mc_oneshot;
use rquickjs::{
    atom::PredefinedAtom,
    class::{Trace, Tracer},
    function::OnceFn,
...
impl<'js> AbortSignal<'js> {
    ...
    pub fn timeout(ctx: Ctx<'js>, milliseconds: u64) -> Result<Class<'js, Self>> {
        ...
        #[cfg(all())]
        {
            crate::llrt_timers::set_timeout_interval(
                &ctx,
                cb,
                milliseconds,
                crate::llrt_utils::provider::ProviderType::Timeout,
            )?;
        }
        #[cfg(all(not(all()), any()))]
        {
            use crate::llrt_utils::ctx::CtxExtension;
...
mod tests {
    ...
    use crate::llrt_test::test_async_with;
    ...
    async fn test_abort_signal() {
        test_async_with(|ctx| {
            crate::llrt_abort::init(&ctx).unwrap();
            crate::llrt_timers::init(&ctx).unwrap();
            Box::pin(async move {
